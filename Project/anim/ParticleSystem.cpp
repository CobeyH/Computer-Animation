...
{
	Vector force, f_external, f_spring;
	...
	for (int i = 0; i < Particles.size(); i++)
	{
		...
		// GROUND FORCES (only in the y)
		zeroVector(f_external);
		Vector groundCheck;
		VecSubtract(groundCheck, Particles[i].position, groundPoint);
		if (VecDotProd(groundCheck, groundNormal) < 0)
		{
			f_external[1] = -ground_ks * VecDotProd(groundCheck, groundNormal) * groundNormal[1]
				- ground_kd * VecDotProd(Particles[i].velocity, groundNormal) * groundNormal[1];
		}
		...
		// FORCES
		force[0] = -kdrag * Particles[i].velocity[0] + f_external[0] + f_spring[0];
		force[1] = -kdrag * Particles[i].velocity[1] + Particles[i].mass * gravity + f_external[1] + f_spring[1];
		force[2] = -kdrag * Particles[i].velocity[2] + f_external[2] + f_spring[2];
		...
		// VELOCITIES
		Particles[i].velocity[0] = Particles[i].velocity[0] + delta_t * force[0] / Particles[i].mass;
		Particles[i].velocity[1] = Particles[i].velocity[1] + delta_t * force[1] / Particles[i].mass;
		Particles[i].velocity[2] = Particles[i].velocity[2] + delta_t * force[2] / Particles[i].mass;
	}
...
void ParticleSystem::verlet(double delta_t)
{
	...
	for (int i = 0; i < Particles.size(); i++)
	{
		...
		Particles[i].position[0] = 2 * Particles[i].position[0] - Particles[i].position_prev[0] + (force[0] / Particles[i].mass) * delta_t * delta_t;
		Particles[i].position[1] = 2 * Particles[i].position[1] - Particles[i].position_prev[1] + (force[1] / Particles[i].mass) * delta_t * delta_t;
		Particles[i].position[2] = 2 * Particles[i].position[2] - Particles[i].position_prev[2] + (force[2] / Particles[i].mass) * delta_t * delta_t;
